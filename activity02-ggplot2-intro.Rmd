...
**Response**: Adding the colo option in aes adds outlines each boxplot with a color representing the specified class in this case species. 
...
**Response**: You can easily see the 5-number summary (min, Q1, median, Q3, max) when creating a boxplot. The jitterplot shows the individual observations. 
...
**Response**:
The md file is much easier to read as it is more of output where the rmd file shows all of the code and comments in one document.
...
**Response**: The options can effect all layers in the in the ggplot global option or only effect a specific layer. 
...
**Response**:
Species seems to have the largest impact on bill length but sex also seems to have a significant difference. 
...
**Response**: As flipper length increases body mass also tends to increase. 
...
**Response**: Changing the alpha to .5 makes the points easier to see as they become more see through.
